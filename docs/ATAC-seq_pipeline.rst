...
The results of the pipeline will be available under the path set using the ``outdir`` parameter once the executions is finished. The nf-core
website provides a thoroughly description of all the outputs that can be generated by the nf-core/atacseq pipeline.
See `here <https://nf-co.re/atacseq/dev/output>`__. 
